//
//  UIViewController+DNTopBarMessage.m
//  DNCategory
//
...
#import "UIViewController+DNTopBarMessage.h"
#import <objc/runtime.h>

#define kTopBarMHeight 38.0f
...
@implementation UIViewController (DNTopBarMessage)
...
- (void)showTopMessage:(NSString *)message topBarConfig:(NSDictionary *)config mode:(TopBarMessageMode)messageMode dismissDelay:(float)delay withTapBlock:(dispatch_block_t)tapHandler {
    ...
    if (!topV) {
        topV = [[TopWarningView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.view.bounds), kTopBarMHeight)];
    ...
    topV.frame = CGRectMake(0, startY, CGRectGetWidth(self.view.bounds), kTopBarMHeight);
    ...
    if (messageMode == TopBarMessageModeResize) {
        [UIView animateWithDuration:0.25f animations:^{
            CGAffineTransform transform = CGAffineTransformMakeTranslation(0, kTopBarMHeight);
            self.view.transform         = transform;
            CGRect frame                = topV.frame;
            frame.origin.y              = -kTopBarMHeight;
            topV.frame                  = frame;
        }];
    }
}
